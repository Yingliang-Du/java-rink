...
public class NumberField {
	private static final Logger LOGGER = Logger.getLogger(NumberField.class);
	...
	public static String format(Number number, String pattern, Locale locale, NumberFormatOption option) {
		// If number value is null - return
		if (number == null) {
			LOGGER.warn("Return null when number value is null");
			return null;
		}
		
		// Use integer format when pattern is null
		if (pattern == null) {
			LOGGER.warn("Use integer format when pattern is null");
			pattern = "integer";
		}
	...
	private static NumberFormat getFormat(String pattern, Locale locale) {
		...
		try {
			...
			LOGGER.warn("Format pattern: " + pattern + " error: " + ex.getMessage());
		}
